...
PID myPID(&Input, &Output, &Setpoint, 0, 0, 0, REVERSE);
...
ISR(TIMER2_OVF_vect) {  // every 22.52 ms
	...
	//Setpoint = 100;
	// error = map(L - R, -L_MAX, R_MAX, 0, 255);
	error = map(5.0*L2 + 2.5*L - 2.5*R - 5.0*R2, -5.0*R2_MAX, 5.0*L2_MAX, 0, 255);
	Input = (double)error;
	...
	L_Speed = SPEED_t + Output;
	R_Speed = SPEED_t - Output;
